...
// Hacer la base de datos accesible globalmente para la sesión Admin
window.db = db;
  ...
  if (isRegistering) {
    ...
    try {
      ...
      await db.ref(`users/${cred.user.uid}`).set({
        ...
        email: email,
        isAdmin: false, 
        status: 'pending', // Registro seguro en modo pendiente
        canEdit: false,
        canDelete: false,
        requestEdit: false
      });
...
async function adminCreateUser() {
  ...
  try {
    // Instancia secundaria para autenticar sin cerrar la sesión admin activa
    ...
    // Desconectar la instancia secundaria para evitar interferencias
    await secondaryAuth.signOut();
    await secondaryApp.delete();

    // Guardar en Realtime Database usando las credenciales del Administrador principal
...
// Exponer funciones globalmente para evitar fallos de alcance con los botones HTML
