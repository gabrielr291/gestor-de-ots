...
// Base de datos expuesta de manera global para la sesión Admin
window.db = db;
  ...
  if (isRegistering) {
    ...
    try {
      ...
      await db.ref(`users/${cred.user.uid}`).set({
        uid: cred.user.uid,
        username: emailInput.split('@')[0].toLowerCase(),
        email: email,
        isAdmin: false, 
        status: 'pending',
        canEdit: false,
      ...
      alert('¡Usuario registrado! Tu cuenta está pendiente de aprobación por el administrador.');
    } catch (err) {
      console.error(err);
...
async function adminCreateUser() {
  ...
  try {
    ...
    await db.ref(`users/${newUid}`).set({
      uid: newUid,
      username: newUser.split('@')[0].toLowerCase(),
      email: email,
      isAdmin: false,
      status: 'active',
      canEdit: true,
...
function renderUsersTable() {
  ...
  tbody.innerHTML = users.map(u => `
    <tr>
      ...
      <td><span style="color:${u.status === 'blocked' ? 'red' : (u.status === 'pending' ? 'orange' : 'green')}">${escapeHTML(u.status)}</span></td>
      <td>
      ...
      <td>
        ${!u.isAdmin ? `
          <button class="btn btn-secondary" onclick="toggleUserStatus('${u.uid}', '${u.status}')">
            ${u.status === 'blocked' || u.status === 'pending' ? 'Activar' : 'Bloquear'}
          </button>
        ` : 'N/A'}
      </td>
    </tr>
...
function toggleUserStatus(uid, currentStatus) {
  const newStatus = (currentStatus === 'blocked' || currentStatus === 'pending') ? 'active' : 'blocked';
...
// BÚSQUEDA AVANZADA MULTICAMPO (Palabra Clave + Bus)
function renderOTs() {
  if (!currentUserProfile) return;

  let ots = Object.values(globalOTs);

  // Obtener valores de los inputs de búsqueda
  const filterBusInput = document.getElementById('searchBus');
  const filterKeywordInput = document.getElementById('searchKeyword');

  const filterBus = filterBusInput ? filterBusInput.value.trim().toLowerCase() : '';
  const filterKeyword = filterKeywordInput ? filterKeywordInput.value.trim().toLowerCase() : '';

  const list = document.getElementById('otList');
  if (!list) return;

  // Filtrar si el usuario no es admin
  if (!currentUserProfile.isAdmin) {
    ots = ots.filter(x => x.createdUid === activeUser.uid);
  }

  // Filtrado compuesto
  let filtered = ots.filter(x => {
    // 1. Filtro por Bus (si tiene texto)
    const matchesBus = !filterBus || (x.bus || '').toLowerCase().includes(filterBus);

    // 2. Filtro por Palabra Clave (en OT, Bus, KM, Detalle, Creador o Extras)
    let matchesKeyword = true;
    if (filterKeyword) {
      const otText = (x.ot || '').toLowerCase();
      const busText = (x.bus || '').toLowerCase();
      const kmText = (x.km || '').toLowerCase();
      const detText = (x.detalle || '').toLowerCase();
      const userText = (x.createdUser || '').toLowerCase();
      
      let extrasText = '';
      if (x.extras) {
        extrasText = Object.entries(x.extras)
          .map(([k, v]) => `${k} ${v}`)
          .join(' ')
          .toLowerCase();
      }

      matchesKeyword = otText.includes(filterKeyword) ||
                       busText.includes(filterKeyword) ||
                       kmText.includes(filterKeyword) ||
                       detText.includes(filterKeyword) ||
                       userText.includes(filterKeyword) ||
                       extrasText.includes(filterKeyword);
    }

    return matchesBus && matchesKeyword;
  });
  ...
  if (!filtered.length) {
    list.innerHTML = `<li style="text-align:center; color:var(--text-muted); padding:1rem;">Sin registros coincidentes.</li>`;
    return;
  }
...
window.toggleUserPerm = toggleUserPerm;
window.toggleUserStatus = toggleUserStatus;
window.checkStrength = checkStrength;
window.toggleSection = toggleSection;
window.renderOTs = renderOTs;