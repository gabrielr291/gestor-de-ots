...
function init() {
  // 1. Cargar sesión local previa si existe
  try {
  ...
  // 2. Listener de Usuarios en tiempo real
  db.ref('users').on('value', (snapshot) => {
    ...
    // Si hay sesión activa, sincronizar el rol real con Firebase y refrescar la vista
    if (activeUser) {
      const refreshedUser = globalUsers[activeUser.username];
      if (refreshedUser) {
        activeUser = refreshedUser;
        showApp(); // Re-renderiza asegurando la visibilidad del panel de Admin
      } else {
        logout(); // Si el usuario fue eliminado mientras estaba conectado
      }
    } else {
      showLogin();
    }
